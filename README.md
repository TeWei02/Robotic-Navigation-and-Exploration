...
comparison is shown on the page.

The same comparison can be run from a terminal, without a browser, with the harness in
`tools/verify_engine.js`. It checks the Python/JavaScript port in both directions — it replays the
recorded commands and it also lets the JavaScript controller re-drive each episode — against the
committed episodes, including the per-step diagnostics and the termination reason:

```bash
jsc tools/verify_engine.js    # JavaScriptCore, ships with macOS
node tools/verify_engine.js   # or Node.js
```

Expected output on the current revision: `scenarios checked: 6` followed by `RESULT: PASS`.

The two implementations are not bit-identical. The committed episodes are rounded to six decimals
when they are written, and NumPy and JavaScript evaluate the same equations in a different
floating-point order (the page replays the recorded commands, while the closed-loop mode
recomputes them in the browser). The harness therefore compares within an explicit tolerance and
prints the residual it measured rather than asserting a fixed value:

| Compared series | Worst measured deviation |
|-----------------|--------------------------|
| Position along the trajectory (replay and closed loop) | `2.1e-6` units |
| Yaw-rate commands (closed loop vs recorded) | `1.6e-6` |
| Reward per step | `2.1e-7` |
| Observation vector | `8.1e-7` |
| Squared distance to path / heading error | `3.8e-5` (on distances of the order of `10²`) |

These are arithmetic residuals of the same equations, not behavioural differences: every episode
ends in the same state and with the same termination reason under both implementations.
...
| `tools/` | `build_web_data.py` — regenerates and verifies the published data set; `verify_engine.js` — offline parity harness for the published JavaScript engine | runnable |
...
# 4. Verify the published JavaScript engine against the committed episodes
#    (JavaScriptCore ships with macOS, so no Node.js install is required)
jsc tools/verify_engine.js

# 5. Serve the published site locally
python3 -m http.server 8000 --directory docs   # http://localhost:8000

# 6. Train the PPO agent (PyTorch; writes checkpoints to HW3-1/save/, not distributed here)
...
Every figure shown on the published page is generated by code in this repository. Four
...
├── tools/verify_engine.js       # Offline parity harness: browser engine vs committed episodes
